...
function Temp() {
  ...
  const addRandomEntityToRandomScene = () => {
    ...
    const randomSceneId =
      listScenesId[Math.floor(Math.random() * listScenesId.length)];
    const entityId = (Math.random() * 10000).toFixed(0);
    engineStore.addEntityToScene(randomSceneId, {
      id: entityId,
  ...
  return (
    <main className="h-screen w-full flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <Button onClick={addRandomEntityToRandomScene}>
          Add Random Entity to Random Scene
        </Button>
        <Button onClick={handleSaveHlz}>Save Project as .hlz</Button>
        <pre className="h-max overflow-auto">
          {JSON.stringify(engineStore, null, 2)}
        </pre>
      </div>
    </main>
  );
